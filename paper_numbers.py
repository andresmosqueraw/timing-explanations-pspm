...
def main():
    ...
    for tag in ("act", "wait"):
        ...
        if "anchored_top" in c:
            out += [macro(f"CARD{T}ANCHOREDTOP", ", ".join(f"\\texttt{{{tex(a)}}} ({v:+.2f})" for a, v in c["anchored_top"][:4])),
                    macro(f"CARD{T}DIRECTTOP", ", ".join(f"\\texttt{{{tex(a)}}} ({v:+.2f})" for a, v in c["direct_top"][:4])),
    ...
    ro_path = paths.REPO / "compose_results_risk_retrained.json"
    if ro_path.exists():
        ro = {x["log"]: x for x in json.loads(ro_path.read_text())["results"]}[LOG]
        rr = ro["readings"]["rebuilt"]
        out += [macro("RONWAIT", str(rr["n_wait"])), macro("RONACT", str(rr["n_act"]))]
        for side in ("act", "wait"):
            S = side.upper()
            ls = rr["level_shares"][side]["per_level"]
            t = rr["propagation"][side]
            out += [macro(f"RORISKSHARE{S}", pct(ls.get("risk", 0.0))), macro(f"RONATIVESHARE{S}", pct(ls.get("native", 0.0))),
                    macro(f"ROCHANRISK{S}", pct(t["channel_share"]["risk"])), macro(f"ROCHANNATIVE{S}", pct(t["channel_share"]["native"])),
                    macro(f"ROTOP{S}", ", \\allowbreak ".join(f"\\texttt{{{tex(d['input'])}}} ({pct(d['share'], 0)})" for d in t["top"][:4])),
                    macro(f"ROTOPFIVE{S}", pct(t["prefix_share_top5"], 0)), macro(f"ROCANCEL{S}", pct(t["cancellation_overall"], 0))]
            if side in ro.get("deletion_e2e", {}):
                out += [macro(f"ROZPROP{S}", num(ro["deletion_e2e"][side]["1"]["propagated"]["z"], 0)), macro(f"ROZDIRECT{S}", num(ro["deletion_e2e"][side]["1"]["direct"]["z"], 0))]
        if "direct" in ro:
            ag = ro["direct"]["agreement_with_propagated"]["all"]
            out += [macro("ROAGREESPEARMAN", num(ag["spearman_mean"], 2)), macro("ROAGREEGLOBAL", num(ag["global_spearman"], 2)), macro("ROAGREETOPONE", pct(ag["top1_agreement"], 0))]
        out.append(macro("ROWDRISK", pct(ro["well_defined"]["risk (r)"]["share_defined"], 0)))
        gain_ro = paths.REPO / "gain_table_results_risk_retrained.json"
        if gain_ro.exists():
            g = json.loads(gain_ro.read_text())["bpic2017"]
            out += [macro("ROINTERVENERATE", pct(g["policy_intervene_rate"], 1)), macro("ROGAIN", num(g["policy_gain"], 1)),
                    macro("ROPRECISION", num(g["policy_precision"], 2) if g["policy_precision"] is not None else "--"), macro("RORECALL", num(g["policy_recall"], 2))]
