...
def main():
    ...
    if ro_path.exists():
        ...
        side = "wait" if rr["n_wait"] >= rr["n_act"] else "act"
        ls = rr["level_shares"][side]["per_level"]
        t = rr["propagation"][side]
        out += [macro("RONWAIT", str(rr["n_wait"])), macro("RONACT", str(rr["n_act"])), macro("ROSIDE", side),
                macro("RORISKSHARE", pct(ls.get("risk", 0.0))), macro("RONATIVESHARE", pct(ls.get("native", 0.0))),
                macro("ROCHANRISK", pct(t["channel_share"]["risk"])), macro("ROCHANNATIVE", pct(t["channel_share"]["native"])),
                macro("ROTOP", ", ".join(f"\\texttt{{{tex(d['input'])}}} ({pct(d['share'], 0)})" for d in t["top"][:4])),
                macro("ROTOPFIVE", pct(t["prefix_share_top5"], 0)), macro("ROCANCEL", pct(t["cancellation_overall"], 0))]
        if "direct" in ro:
        ...
        if side in ro.get("deletion_e2e", {}):
            out += [macro("ROZPROPKONE", num(ro["deletion_e2e"][side]["1"]["propagated"]["z"], 0)), macro("ROZDIRECTKONE", num(ro["deletion_e2e"][side]["1"]["direct"]["z"], 0))]
        wd = ro["well_defined"]
        out.append(macro("ROWDRISK", pct(wd["risk (r)"]["share_defined"], 0)))
